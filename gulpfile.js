...
const config = {			// Initializing the browser-sync package
...
function color(code) {		// Coloring of logs
	if (coloring) console.log(code);
}

function speak(message) {	// Voice action
	if (voice) say.speak(message);
}

function notice(message) {	// Notifications
